...
sigma_a_CV = 1.9  # From EKF exc 3
sigma_a_CT = 0.15  # From EKF exc 3
sigma_omega = 0.002
...
p22 = 0.95
               # CV    CT
PI = np.array([[p11, 1-p11],   # CV
               [1-p22, p22]])  # CT
init_weights = np.array([0.7, 0.3])
...
if writeValuesToFile:
    ...
    append_new_line(log_file, f"PI11 = {p11}. PI22 = {p22}")
    append_new_line(log_file, f"init mode prob: CV: {init_weights[0]}. CT: {init_weights[1]}")
