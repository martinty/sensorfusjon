...
CT = dynamicmodels.ConstantTurnrate(sigma_a, sigma_omega)
measurement_model = measurementmodels.CartesianPosition(sigma_z)
ekf_filter = ekf.EKF(CT, measurement_model)

tracker = pda.PDA(ekf_filter, clutter_intensity, PD, gate_size)

# initialize
z_0 = Z[0]
x_bar_init = np.array([*z_0[0], 0, 0, 0])

P_bar_init = np.zeros((5, 5))
P_bar_init[[0, 1], [0, 1]] = 2 * sigma_z ** 2
P_bar_init[[2, 3], [2, 3]] = 5 ** 2
P_bar_init[4, 4] = 0.0001 ** 2  # 5 th diag element
...
# NIS
NIS = np.array([tracker.state_filter.NIS(zk[0], pred_k) for zk, pred_k in zip(Z, tracker_predict_list)])
ANIS = NIS.mean()
